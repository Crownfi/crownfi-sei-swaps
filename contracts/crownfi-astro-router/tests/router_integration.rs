...
fn router_contract() -> Box<dyn Contract<Empty>> {
    Box::new(ContractWrapper::new_with_empty(
        crownfi_astro_router::contract::execute,
        crownfi_astro_router::contract::instantiate,
        crownfi_astro_router::contract::query,
    ))
}
...
fn router_does_not_enforce_spread_assertion() {
    ...
    let err = app
        ...
        .unwrap_err();
    assert_eq!(
        crownfi_astro_pair::error::ContractError::MaxSpreadAssertion {},
        err.downcast().unwrap()
    )
}
