...
use cosmwasm_std::{attr, Addr, Api, CustomQuery, DepsMut, Env, MessageInfo, Response, StdError, StdResult};
	...
	C: CustomQuery,
{
	...
	Ok(Response::new().add_attributes(vec![attr("action", "propose_new_owner"), attr("new_owner", new_owner)]))
}
	...
	C: CustomQuery,
{
	...
	Ok(Response::new().add_attributes(vec![attr("action", "claim_ownership"), attr("new_owner", p.owner)]))
}
