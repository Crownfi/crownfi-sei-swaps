...
use cosmwasm_std::{
	ConversionOverflowError, Decimal, Decimal256, Fraction, StdError, StdResult, Uint128, Uint256, Uint64,
};
...
impl ConvertInto<Decimal> for Decimal256 {
	...
	fn conv(self) -> Result<Decimal, Self::Error> {
		...
		Decimal::from_atomics(numerator, self.decimal_places()).map_err(|err| StdError::generic_err(err.to_string()))
	}
}
