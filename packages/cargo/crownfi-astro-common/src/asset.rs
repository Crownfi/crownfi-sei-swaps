...
use cosmwasm_std::{
	coin, coins, ensure, to_json_binary, wasm_execute, Addr, Api, BankMsg, Coin, ConversionOverflowError, CosmosMsg,
	CustomMsg, CustomQuery, Decimal256, Fraction, MessageInfo, QuerierWrapper, ReplyOn, StdError, StdResult, SubMsg,
...
impl Asset {
	...
	pub fn assert_sent_native_token_balance(&self, message_info: &MessageInfo) -> StdResult<()> {
		if let AssetInfo::NativeToken { denom } = &self.info {
			let amount = must_pay(message_info, denom).map_err(|err| StdError::generic_err(err.to_string()))?;
	...
	pub fn as_coin(&self) -> StdResult<Coin> {
		match &self.info {
			AssetInfo::Token { .. } => Err(StdError::generic_err("Cannot convert token asset to coin")),
...
pub trait CoinsExt {
	fn assert_coins_properly_sent(&self, assets: &[Asset], pool_asset_infos: &[AssetInfo]) -> StdResult<()>;
}

impl CoinsExt for Vec<Coin> {
	fn assert_coins_properly_sent(&self, input_assets: &[Asset], pool_asset_infos: &[AssetInfo]) -> StdResult<()> {
		ensure!(!input_assets.is_empty(), StdError::generic_err("Empty input assets"));
...
/// # use crownfi_astro_common::asset::AssetInfo::{NativeToken, Token};
...
impl KeyDeserialize for &AssetInfo {
	...
	fn from_vec(_value: Vec<u8>) -> StdResult<Self::Output> {
		unimplemented!(
			"Due to lack of knowledge of enum variant in binary there is no way to determine correct AssetInfo"
		)
	}
}
...
impl AssetInfo {
	...
	pub fn query_pool<C>(&self, querier: &QuerierWrapper<C>, pool_addr: impl Into<String>) -> StdResult<Uint128>
	where
		C: CustomQuery,
	{
		match self {
			AssetInfo::Token { contract_addr, .. } => query_token_balance(querier, contract_addr, pool_addr),
	...
	pub fn equal(&self, asset: &AssetInfo) -> bool {
		match (self, asset) {
			(AssetInfo::NativeToken { denom }, AssetInfo::NativeToken { denom: other_denom }) => denom == other_denom,
			(
				AssetInfo::Token { contract_addr },
				AssetInfo::Token {
...
impl PairInfo {
	...
	pub fn query_pools<C>(&self, querier: &QuerierWrapper<C>, contract_addr: impl Into<String>) -> StdResult<Vec<Asset>>
	where
		C: CustomQuery,
	{
...
pub fn format_lp_token_name<C>(asset_infos: &[AssetInfo], querier: &QuerierWrapper<C>) -> StdResult<String>
where
	C: CustomQuery,
{
	...
	for asset_info in asset_infos {
		let short_symbol = match &asset_info {
			AssetInfo::NativeToken { denom } => denom.chars().take(TOKEN_SYMBOL_MAX_LENGTH).collect(),
			AssetInfo::Token { contract_addr } => {
...
pub fn pair_info_by_pool(querier: &QuerierWrapper, pool: impl Into<String>) -> StdResult<PairInfo> {
	...
	let pair_info: PairInfo = querier.query_wasm_smart(minter_info.minter, &AstroPairQueryMsg::Pair {})?;
...
pub trait Decimal256Ext {
	...
	fn checked_multiply_ratio(&self, numerator: Decimal256, denominator: Decimal256) -> StdResult<Decimal256>;

	fn with_precision(value: impl Into<Uint256>, precision: impl Into<u32>) -> StdResult<Decimal256>;
}
...
impl Decimal256Ext for Decimal256 {
	...
	fn to_uint128_with_precision(&self, precision: impl Into<u32>) -> StdResult<Uint128> {
		...
		value
			...
			.try_into()
			.map_err(|o: ConversionOverflowError| StdError::generic_err(format!("Error converting {}", o.value)))
	}
...
mod tests {
	...
	fn test_proper_native_coins_sent() {
		...
		assert_eq!(
			...
			StdError::generic_err(
				"Native token balance mismatch between the argument (100uluna) and the transferred (0uluna)"
			)
		);
		...
		let err = vec![coin(1000, "uusd"), coin(100, "random")]
			.assert_coins_properly_sent(&assets, &pool_asset_infos)
			.unwrap_err();
		assert_eq!(err, StdError::generic_err("Asset random is not in the pool"));
		...
		assert_eq!(
			err,
			StdError::generic_err("Supplied coins contain uusd that is not in the input asset vector")
		);
	}
	...
	fn test_empty_funds() {
		...
		let err = vec![].assert_coins_properly_sent(&[], &pool_asset_infos).unwrap_err();
	...
	fn test_duplicated_funds() {
		...
		let err = vec![assets[0].as_coin().unwrap(), assets[1].as_coin().unwrap()]
			.assert_coins_properly_sent(&assets, &pool_asset_infos)
			.unwrap_err();
		assert_eq!(err.to_string(), "Generic error: Duplicated assets in the input");
	}

	#[test]
	fn native_denom_validation() {
		let err = validate_native_denom("ab").unwrap_err();
		assert_eq!(err, StdError::generic_err("Invalid denom length [3,128]: ab"));
		...
		assert_eq!(
			err,
			StdError::generic_err("Not all characters are ASCII alphanumeric or one of:  /  :  .  _  -: wow@usd")
		);
		...
		validate_native_denom("ibc/EBD5A24C554198EBAF44979C5B4D2C2D312E6EBAB71962C92F735499C7575839").unwrap();
