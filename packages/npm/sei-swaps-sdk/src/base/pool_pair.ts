 ...
 */
import {Addr, ArraySize_2Of_Coin, ArraySize_2Of_String, Binary, Decimal, ExchangeRateQueryResponse, PoolPairCalcNaiveSwapResult, PoolPairCalcSwapResult, PoolPairConfigJsonable, PoolPairExecuteMsg, PoolPairQueryMsg, PoolPairQuerySimulateDepositResponse, String, Uint128, VolumeQueryResponse} from "./types.js";
...
Data older than 30 days is not guaranteed. */
	...
	/** If past_hours is specified and is greater than 0, returns exchange rate stats for the past specified hours. e.g. 24 means price stats over the past 24 hours, updated every hour (UTC). Each of the 24 bits of the significand (including the implicit 24th bit), bit 23 to bit 0, represents a value, starting at 1 and halves for each bit, as follows: Otherwise, returns the price stats since this hour started (UTC).

Data older than 24 hours is not guaranteed.

The stored encoding for highest and lowest price is lossy and gets more accurate as the exhange rate approaches 1:1 */
	queryExchangeRateHourly(args: {
		"past_hours"?: number | null
	} = {}): Promise<ExchangeRateQueryResponse> {
		const msg = {"exchange_rate_hourly": args} satisfies PoolPairQueryMsg;
		return this.query(msg);
	}
	/** If past_days is specified and is greater than 0, returns exchange rate stats for the past specified days. e.g. 7 means price stats over the past 7 days, updated every midnight (UTC).
...
The stored encoding for highest and lowest price is lossy and gets more accurate as the exhange rate approaches 1:1 */
	queryExchangeRateDaily(args: {
		"past_days"?: number | null
	} = {}): Promise<ExchangeRateQueryResponse> {
		const msg = {"exchange_rate_daily": args} satisfies PoolPairQueryMsg;
		return this.query(msg);
	}
	/** Returns the average, highest, and lowest price */
	queryExchangeRateAllTime(): Promise<ExchangeRateQueryResponse> {
