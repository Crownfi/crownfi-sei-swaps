...
import { ClientEnv, UIAmount, bigIntToStringDecimal, getUserTokenInfo, isProbablyTxError, makeTxExecErrLessFugly, seiUtilEventEmitter, stringDecimalToBigInt } from "@crownfi/sei-utils";
	...
	constructor() {
		...
		this.refs.form.addEventListener("submit", (ev) => {
			...
			const inAmount = stringDecimalToBigInt(
				...
				getUserTokenInfo(inDenom).decimals
			) || 0n;
			errorDialogIfRejected(async () => {
				try{
					...
					await client.executeContractMulti(
						swapMarket.buildSwapIxs(
							inAmount,
							inDenom,
							outDenom,
							0.5
		...
		if (!this.#refreshingTradeOutput) {
			...
			(async () => {
				...
				do {
					...
					const swapMarket = await this.swapMarket();
					const simResult = await swapMarket.simulateSwap(
						client,
						tokenInAmount,
						inDenom,
						outDenom,
					...
					if (approxSlippageRate > 0.005) {
						this.refs.outError.innerText = "Warning: approx. " + (approxSlippageRate * 100).toFixed(2) + "% slippage";
					}else{
						this.refs.outError.innerText = "";
					}
